...
def run_mobile_demo_ADE(selected_walls=None):
    if selected_walls is None:
        selected_walls = ["A", "D", "E"]

    selected_walls = [str(w).upper().strip() for w in selected_walls]

    print("\n" + "-" * 60)
    print("Mobile Base starting now - ADE scenario")
    print(f"Selected walls from coordinator: {selected_walls}")
    print("Running WallsADE mobile base code...")
    print("-" * 60)
    ...
    return {
        ...
        "note": "WallsADE.py main() executed successfully"
    }
